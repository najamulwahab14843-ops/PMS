/**
 * Activation Management System backend — file-storage version (no database needed).
 *
 * MANAGER LOGINS
 * ---------------
 * Manager accounts live directly in this file, in the MANAGER_USERS array
 * below. Passwords are NOT stored in plaintext — each entry is a random
 * salt + a one-way hash of the password (scrypt, built into Node, no extra
 * package needed). This means:
 *   - Nothing in this file can be used to read the real password back out.
 *   - You can safely commit this file to a PUBLIC GitHub repo — the hash
 *     is not the password.
 *   - BUT: don't rely on this alone if your repo is public and the data
 *     matters. Anyone can see how many managers exist and their usernames.
 *     For real privacy, make the GitHub repo Private (free on GitHub for
 *     personal accounts) in addition to this.
 *
 * TO ADD A NEW MANAGER:
 *   1. Run:  node generate-manager-hash.js
 *   2. Enter the username and password when prompted.
 *   3. It prints a line like:
 *        { username: 'ali', salt: '...', hash: '...' },
 *      Paste that line into the MANAGER_USERS array below.
 *   4. Save, commit, redeploy (or restart the server locally).
 *
 * TO REMOVE A MANAGER: delete their line from the array below.
 *
 * (There's no database, so this list only changes when you edit this file
 * and restart/redeploy the server — there's no "Manage Team" button in the
 * app for this version, since there's nowhere persistent to save changes
...
// On Vercel the deployed code is read-only — only /tmp is writable, and /tmp
// is wiped on every cold start / new instance. That means data here is NOT
// persistent in production; it's only here so the app doesn't crash on save.
// See the deployment notes for a real database if you need data to stick.
...
// Vercel runs this as a serverless function and calls the exported app directly —
// app.listen() below only matters when you run `node api/index.js` locally.
if (require.main === module) {
  app.listen(PORT, () => {
    console.log(`Activation Management System server running at http://localhost:${PORT}`);
